...
public class StepDefinition {
   ...
   public void userSelectsEnterAsTheSearchOptions(String location, String hotel, String room_type, String NOR, String CID, String COD, String APR, String CPR) {
    ...
    @Then("Select hotel page should open {string}, {string}, {string}, {string}, {string}, {string}")
    public void selectHotelPageShouldOpen(String location, String hotel, String room_type, String NOR, String CID, String COD) throws InterruptedException {

        Thread.sleep( 1000);
        WebElement Result = driver.findElement(By.id("hotel_name_0"));
        String resultText = Result.getAttribute("value");
        Assert.assertTrue("Search term not found in search results", resultText.contains(hotel));

        Result=(driver.findElement(By.id("location_0")));
        resultText = Result.getAttribute("value");
        Assert.assertTrue("Search term not found in search results", resultText.contains(location));

        //can't verify this yet because the field names/value names are different
        //Result=driver.findElement(By.id("rooms_0"));
        //resultText = Result.getAttribute("value");
        //Assert.assertTrue("Search term not found in search results", resultText.contains(NOR));

        Result=driver.findElement(By.id("arr_date_0"));
        resultText = Result.getAttribute("value");
        Assert.assertTrue("Search term not found in search results", resultText.contains(CID));

        Result=(driver.findElement(By.id("dep_date_0")));
        resultText = Result.getAttribute("value");
        Assert.assertTrue("Search term not found in search results", resultText.contains(COD));

        //searchResultElements.add(driver.findElement(By.id("no_days_0")));
        //resultText = Result.getAttribute("value");
        //Assert.assertTrue("Search term not found in search results", resultText.contains(COD));

        Result=(driver.findElement(By.id("room_type_0")));
        resultText = Result.getAttribute("value");
        Assert.assertTrue("Search term not found in search results", resultText.contains(room_type));
    ...
    @When("User clicks the Booked itinerary link")
    public void userClicksTheBookedItineraryLink() {
        WebElement element = driver.findElement(By.xpath("//a[@href='BookedItinerary.php']"));
        element.click();
    }

    @Then("Booked itinerary page opens")
    public void bookedItineraryPageOpens() {
